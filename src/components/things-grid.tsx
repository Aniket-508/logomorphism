...
// Per-millisecond friction (applied as friction^deltaTime)
const FRICTION = 0.997;
// Scale px/ms velocity to px/frame at 60fps
...
const debounce = <T extends (...args: unknown[]) => unknown>(
  ...
  const debouncedFn = function debouncedFn(...args: Parameters<T>) {
    if (timeoutId) {
...
const throttle = <T extends (...args: unknown[]) => unknown>(
  ...
  const throttledFn = function throttledFn(...args: Parameters<T>) {
    const now = Date.now();
  ...
  throttledFn.cancel = function cancel() {
    if (timeoutId) {
      clearTimeout(timeoutId);
      timeoutId = undefined;
    }
...
export interface ThingsGridProps {
  gridSize: number;
...
class ThingsGrid extends Component<ThingsGridProps, State> {
  ...
  constructor(props: ThingsGridProps) {
    super(props);
  ...
  public publicGetCurrentPosition = () => this.state.offset;
  ...
  private static getItemIndexForPosition = (x: number, y: number): number => {
  ...
  private updateGridItems = () => {
    ...
    const newItems = positions.map((position) => {
      const gridIndex = ThingsGrid.getItemIndexForPosition(
        position.x,
        position.y
      );
      return {
  ...
  private handleMove = (p: Position) => {
    ...
    for (let i = 0; i < velocityHistory.length; i += 1) {
      ...
      const weight = 2 ** i;
      totalWeight += weight;
      smoothedX += velocityHistory[i].x * weight;
      smoothedY += velocityHistory[i].y * weight;
    }
    const smoothedVelocity = { x: smoothedX, y: smoothedY };
  ...
  private handleTouchStart = (e: React.TouchEvent) => {
    // eslint-disable-next-line prefer-destructuring -- TouchList is not iterable
